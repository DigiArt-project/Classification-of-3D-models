...
dst = jitter_chunk(train_set) # train_1st_rot_only
print dst.shape
# append augmented data to original data
augmented_train_set = np.append(train_set, dst, axis=0)
augmented_train_labels = np.append(train_labels, train_labels, axis=0) # train_labels, labels_1st_rot_only
...
# N=(n-4)/6
model = ror_wrn.create_pre_residual_of_residual((1, rows, cols, depth), nb_classes=10, N=2, k=2, dropout=0.0)
...
callbacks_list = [callbacks.ModelCheckpoint("RoR-WRN_augm_modified.{epoch:03d}-{val_loss:.5f}-{val_acc:.4f}-{loss:.5f}-{acc:.4f}.hdf5",
                                            monitor="val_loss", verbose=1, save_best_only=False, mode='auto'),
                  callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=5, min_lr=0)
                  ]
...
print("Saving model...")
model.save("ror-wrn_modelnet10_augm_modified_keras.h5")
...
for i in range(0, len(y_val_new), n_rotations):
    ...
    final_proba.append(np.sum(yPreds[i:i+n_rotations, :], axis=0))
    # groundtruth labels
